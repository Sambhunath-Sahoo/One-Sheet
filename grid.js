...
// elements for making font parts
...
let allCells = document.querySelectorAll(".grid .cell");        // used in formula.js also

// sheets arrays for multiple sheets
let sheetArray = [];

// visualize as 2d array -> current sheet
let sheetDB;
// for firstSheet only ->default clicked
firstSheet.addEventListener("click", makeMeActive);
firstSheet.click();

btnContainer.addEventListener("click", function() {

    let AllSheets = document.querySelectorAll(".sheet");     // getting element of all sheets in an array   
    ...
    Lastidx = Number(Lastidx);  // convert string to Number

    // create a new sheet
    let Newsheet = document.createElement("div");
    Newsheet.setAttribute("class", "sheet");
    Newsheet.setAttribute("idx", `${Lastidx + 1}`);   // setting the sheet index as 0+1 
    Newsheet.innerText= `Sheet ${Lastidx + 2}`;   // sheet 0+2 because first sheet doesn't include in our array

    // adding newsheet to the sheetlist and making this active
    sheetList.appendChild(Newsheet);
    for(let i=0; i<AllSheets; i++) {
    ...
    // ------------------multiple sheet--------------------
    // create a new sheet for the grid with empty UI
    createSheet();
    ...
    // new sheet create -> add event listener attached
...
function makeMeActive(e) {
    ...
    // adding active class to current sheet
    sheet.classList.add("active");

    // if add button is clicked, then following will be executed for new sheet
